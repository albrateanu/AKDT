...
class BaseModel():
    ...
    def reduce_loss_dict_old(self, loss_dict):
        """reduce loss dict.
    ...
    def reduce_loss_dict(self, loss_dict):
        ...
        with torch.no_grad():
            if self.opt['dist']:
                ...
                for name, value in loss_dict.items():
                    keys.append(name)
                    # Ensure value is a tensor for distributed reduction
                    value = torch.tensor(value).to(self.device) if not torch.is_tensor(value) else value
                    losses.append(value)
                losses = torch.stack(losses, 0)
                torch.distributed.reduce(losses, dst=0)
                if self.opt['rank'] == 0:
            ...
            log_dict = OrderedDict()
            for name, value in loss_dict.items():
                # Check if value is a tensor to decide on calling .mean()
                if torch.is_tensor(value):
                    log_dict[name] = value.mean().item()
                else:
                    # Value is already a scalar (float)
                    log_dict[name] = value
